	...
	private void initialize() {
		...
		button.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
					...
					try {
						consulta.getservicioPractica1SOAP().validarIBAN(input_IBAN.getText(), soapKeyInput.getText(), respuesta, respuestaError);
						if(respuesta.value){
		...
		button_1.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				try {
					consulta.getservicioPractica1SOAP().consultaCodigoPostal(input_cp.getText(), soapKeyInput.getText(), codigoPostal, poblacion, provincia);
					...
					if(poblacion.value.equals("No se ha encontrado")){
						...
						respuestaCP.setText("El CP " + codigoPostal.value + " se corresponde con la localidad " + poblacion.value + " perteneciente a la provicincia de " + provincia.value);
			...
			public void actionPerformed(ActionEvent arg0) {
				...
				try {
					...
					try {
						...
						respuestaPresupuesto.setText("Ha habido un error, comrpueba que todos los datos son correctos.");

					}	
					System.out.println(d);
					consulta.getservicioPractica1SOAP().generarPresupuesto(date, soapKeyInput.getText(), inputReferencia.getText(), Integer.parseInt(inputCantidad.getValue().toString()),Integer.parseInt(inputID.getText()), idPresupuesto, presupuestoGeneradoCorrectamente);
					...
					if(presupuestoGeneradoCorrectamente.value){
						...
						respuestaPresupuesto.setText("Ha habido algun problema, comprueba que los datos introducidos son correctos." );
					}
				} catch (RemoteException e1) {
					// TODO Auto-generated catch block
		...
		btnComprobar.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				try {
					ServicioPractica1_Service consulta = new ServicioPractica1_ServiceLocator();
					Boolean respuesta = consulta.getservicioPractica1SOAP().validarNIF(input_dni.getText(), soapKeyInput.getText());
					if(respuesta){
